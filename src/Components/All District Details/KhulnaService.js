...
const KhulnaService = () => {
    ...
    useEffect(() => {
      ...
      const khulnaDivision = services.filter(res =>res.division === "Khulna Division")
    ...
    return (
        ...
        <Container>
            <div className="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-4">
                ...
                {khulnaDivision.map(service => <ServiceDetails key={service._id} service={service}/>)}
            </div>
        </Container>
    </div>
    );
