...
const MymensinghService = () => {
    ...
    useEffect(() => {
      ...
      const mymensinghDivision = services.filter(res =>res.division === "Mymensingh Division")
    ...
    return (
        ...
        <Container>
            <div className="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-4">
                ...
                {mymensinghDivision.map(service => <ServiceDetails key={service._id} service={service}/>)}
            </div>
        </Container>
    </div>
    );
...
export default MymensinghService;